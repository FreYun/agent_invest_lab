...
2. **数据调取按 `simulated_today`（目标时点）收口**：只允许返回 `simulated_today` 当天及之前的数据；哪怕上游数据库里有更新的数据，也不允许返回。
...
| `fund_performance` | "近 N 年收益率/回撤/夏普"算到最新日期，无法锚定到 `simulated_today` |
...
所有保留的工具都新增**必填**首参数 `simulated_today: str`，格式 `YYYY-MM-DD`。

**① 已有显式日期参数（15 个）** —— 原样保留入参，新增 `simulated_today`：
...
- `fund_basic_info`：响应含 `成立时间`；按 `成立时间 > simulated_today` 整条剔除。另：响应里 `基金经理`（以及任何明显是"当下值"的字段，如 `最新定期报告时间`）置为 `null`，并在该 dict 同级加 `_pit_note: "字段已按时点屏蔽：基金经理为当前值，非 simulated_today 当时任职人"`。
- `stock_profile`：响应含 `上市日期`；按 `上市日期 > simulated_today` 整条剔除。（申万/中信行业分类变动慢，本期当作准静态，不屏蔽。）

**③ 特殊处理（2 个）**：
- `ttjj_research_search`：把"近 N 天"（`search_days`）重算为时间窗 `[simulated_today - search_days 天, simulated_today]` 后再调上游；返回结果再按发布日期 ≤ `simulated_today` 过滤。
- `market_realtime_quote`：保留 `simulated_today`；响应里的行情时间戳字段（如 `时间`/`更新时间`/`timestamp`）> `simulated_today` 则该条剔除。**已知局限**：`simulated_today` 早于今天时，实时行情几乎必然被全部过滤掉，相当于该工具仅对"今天"有效——会在 docstring 写明。

## 4. `simulated_today` 注入规则（调上游 API 之前）

对每个保留的工具，构造请求体时：

1. **`simulated_today` 自身校验**：解析失败（非 `YYYY-MM-DD`）→ 直接返回 `{"error": "bad_simulated_today", "message": "<原值>"}`，不调上游。
2. **有 `end_date` 入参的工具**（`fund_nav`、`market_index_quote`、`stock_market`、`stock_capital_flow`、`stock_alpha`、`stock_events`、`macro_data`、`commodity_market`、`research_view`）：
   `end_date_final = min(用户传的 end_date（若有，否则取 simulated_today）, simulated_today)`，静默裁剪后传给上游。
3. **有单点日期入参的工具**（`fund_index_return.trade_date`、`fund_bonus.date`、`stock_ownership.report_date`、`stock_financial_quality.trade_date`、`stock_alpha.trade_date`、`stock_market.trade_date`、`bond_yield_curve.date`）：
   - 用户传了且 > `simulated_today` → 返回 `{"error": "lookahead", "message": "<param>=<value> 晚于 simulated_today=<simulated_today>"}`，不调上游；
   - 用户没传 → 填 `simulated_today`。
4. **`start_date` 入参**（`fund_nav`、`fund_abnormal_movement`、`market_index_quote`、`stock_market`、`stock_capital_flow`、`stock_alpha`、`stock_events`、`macro_data`、`commodity_market`、`research_view`）：用户传了且 > `simulated_today` → 返回 `{"error": "lookahead", ...}`，否则原样传。
...
上游返回 JSON 后，进 `_filter_response(obj, simulated_today)` 递归处理，再返回给调用方。

- **遍历到 list**：对其中每个元素，若元素是 dict 且含至少一个"日期型字段"且该字段值能解析成日期：
  - 该日期 > `simulated_today` → 把这个元素从 list 中剔除；
  - 否则保留，并对该元素继续递归。
  - 元素不是 dict（或没有日期型字段）→ 保留，继续递归。
- **遍历到 dict（非某个 list 的元素，或元素递归时）**：
  - 对每个值递归处理；
  - 此外，若 dict 里某个"日期型字段"的值能解析且 > `simulated_today`（典型如顶层 `data.最新净值日期` 之类的标量），把该字段置为 `null`，并在同级加一个 `_pit_truncated: true` 标记。
...
- **例外字段**：`metadata.query_time` / `query_time` / `_simulated_today` 这类"接口元数据时间"不参与过滤（白名单，不当作数据日期）。
- **日期解析容错**：依次尝试 `%Y-%m-%d`、`%Y%m%d`、`%Y/%m/%d`、`%Y-%m-%d %H:%M:%S`、`%Y/%m/%d %H:%M:%S`、`%Y-%m`、`%Y`；都失败 → 认为该字段不是日期，不做任何处理。比较时统一按"日"粒度（带时分秒的截到日）。
- 处理完后在**最外层返回对象**加 `_simulated_today: "<传入的 simulated_today 原值>"`，方便调用方确认时点已生效。
...
├── _filter_response(obj, simulated_today) -> obj # 第 5 节递归过滤
├── _check_simulated_today(simulated_today)            # 第 4.1 校验，返回 (ok: bool, err: dict|None, date_obj)
├── _clamp_end_date(user_end, simulated_today)
├── _reject_if_future(name, value, simulated_today) -> dict|None  # 单点/start_date 检查
├── _post(path, data, simulated_today)       # 调上游 + raise_for_status + success 检查 + _filter_response
├── _err(e)                             # 同 server.py
├── 19 个 @_mcp.tool()                   # 见第 3.2，每个首参 simulated_today
...
```python
@_mcp.tool()
def fund_nav(simulated_today: str, fund_codes: list[str],
             start_date: Optional[str] = None, end_date: Optional[str] = None) -> dict[str, Any]:
    """基金净值历史（时点版：只返回 simulated_today 当天及之前）。"""
    ok, err, simulated_today = _check_simulated_today(simulated_today)
    if not ok:
        return err
    if (e := _reject_if_future("start_date", start_date, simulated_today)):
        return e
    try:
        d: dict = {"fund_codes": fund_codes}
        if start_date:
            d["start_date"] = start_date
        d["end_date"] = _clamp_end_date(end_date, simulated_today)   # = min(end_date or simulated_today, simulated_today)
        return _post("/api/fund/nav", d, simulated_today)
...
- `bad_simulated_today` —— `simulated_today` 格式非法
- `lookahead` —— 用户传入的某个日期参数晚于 `simulated_today`

## 8. 已知局限（同时写进文件头注释）

1. `fund_top_holdings` / `fund_invest_position` / `fund_turnover_rate` / `fund_industry_exposure` 本期未纳入（上游无披露日期、只给最新一期），待后续单独处理。
2. `market_realtime_quote` 在 `simulated_today` 早于今天时几乎必然返回空（实时行情时间戳必然 > 过去的 simulated_today），属预期行为。
3. `fund_basic_info` 的 `基金经理` 等"当下值"字段被置 `null`；本 MCP 不提供"`simulated_today` 当时的真实任职经理"。
...
- `simulated_today` 校验 / `end_date` 裁剪 / 单点日期检查：几次日期解析，微秒级。
...
- 把 `end_date` 收口到 `simulated_today` 通常让上游少返回数据，payload 更小，历史查询往往比原版还快。

## 9. 不做的事（YAGNI）

- 不做全局/启动时配置的截止日期（已定为每个工具的 `simulated_today` 入参）。
...
- `simulated_today` 给一个历史日期（如 `2024-01-01`）调 `fund_nav(["110011"])`，确认返回的 `净值/交易日期` 没有任何 > 2024-01-01 的记录，且顶层有 `_simulated_today`。
- 给 `fund_nav` 传 `end_date="2025-06-01"`、`simulated_today="2024-01-01"`，确认被裁剪到 2024-01-01（不报错）。
- 给 `stock_ownership` 传 `report_date="2025-12-31"`、`simulated_today="2024-01-01"`，确认返回 `{"error": "lookahead", ...}`。
- `simulated_today="2024-13-40"` → 返回 `{"error": "bad_simulated_today", ...}`。
- `market_realtime_quote(simulated_today="2020-01-01", codes=[...])` → 返回（基本为空，不报错）。
