...
h=sqlite3.connect("data/hist_mv.db"); mv=pd.read_sql("SELECT trade_date,ts_code,total_mv FROM hist_mv",h); h.close()
...
mvp=mv.pivot(index="trade_date",columns="ts_code",values="total_mv").sort_index().reindex(index=close.index,columns=close.columns).ffill()
try:
    import akshare as ak
    nm=ak.stock_info_a_code_name(); nm["ts6"]=nm["code"].astype(str).str.zfill(6)
    st6=set(nm.loc[nm["name"].str.contains("ST",na=False),"ts6"]); st=[c for c in close.columns if c.split(".")[0] in st6]
except Exception as _e:
    print("ST名单获取失败(忽略ST过滤):",str(_e)[:50]); st=[]
mask=(mvp>=1e10)&close.notna()&(~close.columns.isin(st))
...
for label,start in [("无偏全样本18+","20180101"),("无偏OOS22-26","20220101")]:
