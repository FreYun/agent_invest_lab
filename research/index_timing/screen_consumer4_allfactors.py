"""对 消费/医药 4 指数跑 local_sweep 全 44 因子 IS/OOS + validation，系统找绿灯。
中证酒 399987 / 800消费 000932 / 生物医药 399441 / 中证医疗 399989（复用 resource4 模板）。
数据从 simworld 18078 拉指数全周期（probe_consumer4.py 已确认仅 close，无 high/low）。"""
...
_,sid=call("initialize",{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"cons4f","version":"1"}})
RES4=[("中证酒","399987.SZ"),("800消费","000932.SH"),
      ("生物医药","399441.SZ"),("中证医疗","399989.SZ")]
syms=[c for _,c in RES4]
res,_=call("tools/call",{"name":"market_index_quote","arguments":{"market":"cn","symbols":syms,"start_date":"2009-01-01","end_date":"2026-06-11","simulated_datetime":"2026-06-11 15:00:00","simulated_today":"2026-06-11"}},sid)
_txt=res["result"]["content"][0]["text"]
try:
    items={it.get("指数标识"):it for it in json.loads(_txt).get("items",[])}
except Exception:
    print("调用返回非JSON，前600字符：\n",_txt[:600]);sys.exit(1)
...
for nm,code in RES4:
    ...
    d0=df.index[0];IS_S=str(d0.date());IS_E="2021-12-31";OOS_S="2022-01-01";OOS_E="2026-06-11"
    ...
    for name,fn,kw,mis,mos in winners[:8]:
