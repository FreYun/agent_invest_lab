...
> 这节管一件 METHODOLOGY 没单独讲的事：你手里**既有"读政策/研报"的叙事能力**（`research_search`），**又有机械因子 + 估值锚**（`quant_factor` 的 `hs300_erp_vix_timing`、市场级 `market_retail_contrarian`，以及你框架自己的趋势 / ERP）——这两类信号该在什么局面听谁。下面只给**判断框架 + 盲区地图，一个硬阈值都不给**：阈值留给 A/B 回测去证伪，不靠我现在拍。

## 一句话心法

**政策催化（onset）时信叙事，趋势尤其狂热时信因子——但"因子"指的是 `retail_contrarian`（真·散户资金狂热）+ 趋势本身是否转弱，不是估值/PE。** 但"现在是不是 onset""是不是狂热顶"由你**当场判**——不给"日期=T/T-1 才算 onset""涨了 X% 才算狂热""ERP z 超某值才减仓"这类线。拿少数样本去拟这些角落值就是过拟合，而且会把你从一个会判断的 agent 退化成 if/then 规则机。

> ⚠️ **A股要害：PE/估值在中国市场、尤其趋势市右侧是反向指标。** 戴维斯双击里估值一路扩张——拿"贵了该减"封顶，会把你砍在主升里。**估值只提示风险刻度（该不该想），永不给兑现扳机（该不该卖）。** 顶部刹车永远是 **资金面狂热（`retail_contrarian` 散户堆量申购）+ 趋势转弱**，不是 PE/ERP 分位。
...
- **因子 / 估值锚（`retail_contrarian`、`hs300_erp_vix_timing`、ERP、趋势）的盲区 = 全新 onset。** 它们都是拿历史价格 / 资金流 / 估值拟的，对**没发生过的政策冲击天生盲**——组合拳落地当晚 ERP 还在低分位（甚至因利率 regime 显"贵"）、retail 还在喊减、MA60 还没站上，跟着它会**踏空主升**。这正是你那条 🔴 **破单向陷阱**（清仓后再进场不许被估值单维否决）要防的东西——**onset 就是"估值/因子说别进、但该进"的典型局面**。
  - 但 `retail_contrarian`（散户堆量申购接盘）有叙事接不住的强项：**散户狂热顶**它最看空，正好补上研报 perma-bull 的盲区。⚠️ 注意刹车是**资金面狂热**，**不是 ERP"贵"**——估值在趋势右侧反向，别把它算进顶部刹车。
...
2. **判这是哪种局面**：
   ...
   - 更像**成熟的狂热趋势 / 顶部**（已涨一大段、散户堆量申购、研报喊"政策底确认/已涨"）→ **让 `retail_contrarian`（真·散户资金狂热）+ 趋势本身是否转弱来刹**，研报越看多越警惕，别被卖方默认偏多带去接盘。**别用 PE/ERP"贵"封顶兑现**——趋势右侧"贵"是反向指标，拿它兑现会砍在主升里；趋势没破、资金面没过热，就让利润跑。
...
- **`hs300_erp_vix_timing` 当顶部刹车要拆两条腿看**：它"减仓"= ERP贵 + VIX自满。**ERP"贵"那条腿在趋势右侧是反向指标**——牛市里 ERP 一直喊贵会让你过早减仓（因子自己的研究都承认"牛市低暴露让出绝对涨幅"），**别信它的"贵"**。真正有用的是 **VIX 低位自满**那条腿（情绪自满预警），且要 VIX 低位才触发；**剧烈高 VIX 逼空顶**它的恐慌腿反而会误读成"抄底"。所以 HS300 顶部刹车的主力是 `retail_contrarian`（真·散户资金狂热），erp_vix 只当"VIX 情绪那半"的辅助。
- 你的承重是**趋势**；**ERP/估值只是风险刻度（上限意识），不是择时扳机**——尤其趋势右侧"贵"反向。催化只在 onset 改"该不该提前进"、`retail_contrarian` 只在散户狂热顶改"该不该兑现"，**趋势转弱才是真正的离场扳机**；估值始终只封顶、不发兑现令。
