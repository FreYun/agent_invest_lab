...
def fund_bonus(simulated_today: str, fund_code: str, date: Optional[str] = None) -> dict[str, Any]:
    ...
    晚于 simulated_today 的分红记录会在响应里被剔除。
    """
    cutoff, err = _check_simulated_today(simulated_today)
    if err:
        return err
    ...
    try:
        ...
        if date:
            d["date"] = date
        return _pit_wrap(_post("/api/fund/bonus", d), cutoff, simulated_today)
    except Exception as e:
        return _err(e)
...
def commodity_market(
    ...
    end_date 会被裁剪到 simulated_today。
    """
    cutoff, err = _check_simulated_today(simulated_today)
    if err:
        return err
...
def bond_yield_curve(
    ...
    maturities: 期限列表, 如 ['10Y','30Y'] 或 ['5y','10y']。end_date 会被裁剪到 simulated_today。
    """
    cutoff, err = _check_simulated_today(simulated_today)
    if err:
        return err
    ...
    try:
        ...
        if start_date:
            d["start_date"] = start_date
        d["end_date"] = _clamp_end_date(end_date, cutoff)
        return _pit_wrap(_post("/api/bond/yield-curve", d), cutoff, simulated_today)
    except Exception as e:
        return _err(e)
