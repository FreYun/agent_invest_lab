...
function setupWorldDir(bots: string[], dates: string[]) {
  ...
  const config: WorldConfig = { researchLoop: '/no', botsRoot, openclawJson, skillsRoot, bots, replay: { from: dates[0], to: dates[dates.length - 1] }, calendar: P.calendarFile(worldRoot), concurrency: 4, perBotTimeoutSeconds: 30, researchDayEvery: 0, researchDayTimeoutSeconds: 300, chatStepDays: 1, chatStepMode: 'trading_days', rlConfigBase: cfgBase, rlOpenclawDir: join(root, 'oc'), shadowInclude: ['SOUL.md'], loop: 'research-loop', simworldUpstreamUrl: 'http://127.0.0.1:1/mcp' }
