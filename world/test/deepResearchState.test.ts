...
test('agent-triggered: Day1 未到间隔，授权自主判断但不强制', () => {
...
test('agent-triggered: 从未深研时第 5 个交易日（gap=4）才强制', () => {
  ...
  for (const [i, d] of [['2025-01-03', 1], ['2025-01-06', 2], ['2025-01-07', 3]].entries()) {
    ...
    assert.equal(s.authorized, true, `day ${d[0]} 应允许异常条件下自主深研`)
...
test('agent-triggered: 配置 7 时 gap=6 自主授权不强制、gap=7 强制', () => {
