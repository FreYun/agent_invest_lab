...
"""把看板所有「合格」的单指数 run 从 07-20 推进到覆盖 07-21。

- 合格口径：停在 2026-07-20 的单日 dash-* run，单 bot ∈ bot1..bot20（排除多指数 bot101/102/103），
  且未被 run-verdicts 手动标 fail。
- 每个 run 内部按 [07-21] 推进（账户逐日结转），幂等：只跑比当前 state 更新的交易日。
- 跨 run 最多 CONC 个并发。逐 run 写日志，末尾写 summary.tsv。
- 计划由 cron 在 23:00 触发（届时当日基金 NAV 已发，可跑全天 decide+close+settle）。
"""
import json, os, glob, re, subprocess, sys, time
from concurrent.futures import ThreadPoolExecutor, as_completed

WORLD = "/home/rooot/agent_invest_lab/world"
PREV_DATE = "2026-07-20"
DATES = ["2026-07-21"]
CONC = 5
LOGDIR = "/tmp/advance-0721"
os.chdir(WORLD)
...
def build_pairs():
    ...
    for f in glob.glob("runtime/runs/*/state.json"):
        try:
            ...
            continue
        if s.get("trading_dates") != [PREV_DATE]:
            continue
