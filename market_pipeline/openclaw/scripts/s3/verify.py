...
def verify_one_backtest(candidate: dict, bars: list[dict]) -> dict:
    ...
    for idx, bar in enumerate(bars[1:MAX_HOLD_DAYS], start=2):
        if bar.get("down_limit") is not None and bar["low"] <= bar["down_limit"] + 0.01:
            continue

        if bar["open"] <= stop_loss:
            return _exit("stop_hit_at_open", entry_status, entry_price, bar["open"], "stop_hit_at_open", bar, idx, t1_open_gap_pct)
        if bar["low"] <= stop_loss:
            return _exit("stop_hit", entry_status, entry_price, stop_loss, "stop_hit", bar, idx, t1_open_gap_pct)
...
def main() -> None:
    ...
    print(f"S3 verify {args.date}: {len(results)} 条")
