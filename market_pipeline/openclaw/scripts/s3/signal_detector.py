...
DEFAULTS = {
    "hot_industry_top_n": 3,
    "min_industry_limit_count": 3,
    "required_streak": 1,
    "max_blast_count": 1,
    "latest_first_seal_time": 140000,
...
def detect_s3(row: dict, hot_by_industry: dict[str, dict], t_bar: Optional[dict], config: Optional[dict] = None) -> dict:
    ...
    if streak != cfg["required_streak"]:
        return _reject("first_board", f"连板数={streak}, 不是首板")
...
def _score(row: dict, hot: dict, first_seal: int | None, t_bar: dict) -> float:
    ...
    turnover_score = 10 if 5 <= turnover <= 18 else 5 if turnover else 3
    amount_score = min(float(row.get("amount") or 0) / 1_000_000_000, 10.0)
    body_score = 5 if t_bar["close"] >= t_bar["open"] else 0
    return round(sector_score + limit_count_score + time_score + blast_score + turnover_score + amount_score + body_score, 2)
