...
_spec = importlib.util.spec_from_file_location("s2_select", os.path.join(SCRIPT_DIR, "select.py"))
_s2_select = importlib.util.module_from_spec(_spec)
assert _spec.loader is not None
_spec.loader.exec_module(_s2_select)
run_select = _s2_select.run_select
...
def run_backtest(
    ...
    for t_date in get_trading_days(start, end):
        ...
        if t1_date is None:
            ...
            results = verify_payload(payload, t1_date) if should_verify else []
    ...
    if output_dir:
        ...
        with open(os.path.join(output_dir, "s2_backtest_summary.json"), "w", encoding="utf-8") as f:
...
def main() -> None:
    ...
    ap.add_argument("--ignore-playbook", action="store_true")
    ap.add_argument("--allow-regimes", default=None)
    ...
    result = run_backtest(args.start, args.end, args.output_dir, args.rules_version, config, tradable_only=not args.include_gate_closed)
    s = result["summary"]
    print(f"S2 回测 {args.start} ~ {args.end}")
