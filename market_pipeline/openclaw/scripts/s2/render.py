...
def main() -> None:
    ...
    if args.cmd == "candidates":
        ...
        if payload is None:
            raise SystemExit(f"DB 中没有 {args.date} 的 S2 select_run")
        _output(render_md(payload), args.output)
    elif args.cmd == "verification":
        payload = read_verification(args.date)
        if payload is None:
            raise SystemExit(f"DB 中没有 {args.date} 的 S2 verification")
        _output(render_verification_md(payload["t1_date"], payload["t_date"], payload["results"], payload["mode"]), args.output)
    else:
        init_market_db()
        conn = get_market_db()
        try:
            print("=== s2_select_runs ===")
            for r in conn.execute("SELECT date, regime_code, passed_count, skipped_reason FROM s2_select_runs ORDER BY date"):
